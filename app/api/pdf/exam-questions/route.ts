...
import { buildSyllabusContentFromCurriculum } from "@/lib/services/syllabus"
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Two valid sources: an uploaded/pasted document, or the learner's
    // syllabus. When generating from syllabus there's no document — the
    // curriculum's own topic list stands in for one.
    let text = rawText
    if (fromSyllabus && (!text || !text.trim()) && subject) {
      text = await buildSyllabusContentFromCurriculum(syllabus, subject, topicFocus).catch(() => null)
      if (!text) {
    ...
    // The legacy pre-atomic quota check (a separate subscriptions lookup +
    // usage_tracking.exams_generated counter) has been removed from here.
    // enforceQuota() above is the authoritative check now — see the 015
    // migration's own comment: it existed specifically to replace this kind
    // of check-in-application-code, which could race and, worse, disagree
    // with the atomic system about whether a request should be allowed.
    // Running both was pure duplication with two different sources of truth.
