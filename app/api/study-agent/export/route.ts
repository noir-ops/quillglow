...
export async function POST(req: Request) {
  ...
  if (target !== "planner" && target !== "notes" && target !== "exam") {
    return NextResponse.json({ error: "target must be 'planner', 'notes' or 'exam'" }, { status: 400 })
  }
  ...
  const { data: session, error: sessionError } = await supabase
    ...
    .select("id, input_text, subject, study_plan, generated_notes, generated_exam")
    .eq("id", sessionId)
    .eq("user_id", user.id)
    .maybeSingle()
