...
export async function POST(req: Request) {
  ...
  if (target === "planner") {
    ...
    // The agent returns at most 7 items — a WEEK's rhythm, not a whole prep
    // window. Tiling it across 4 weeks means one generation gives the
    // calendar a full month of spaced review instead of going empty after
    // day 7. If the model ever returns more than 7 (it's not supposed to,
    // but nothing enforces it), that's respected as-is and just capped at 28
    // total days rather than tiled on top of itself.
    const WEEK_LEN = 7
    const MAX_WEEKS = 4
    const MAX_DAYS = WEEK_LEN * MAX_WEEKS
    const isWeeklyTemplate = schedule.length <= WEEK_LEN
    const totalDays = isWeeklyTemplate ? Math.min(MAX_DAYS, WEEK_LEN * MAX_WEEKS) : Math.min(schedule.length, MAX_DAYS)
    const weekCount = Math.min(MAX_WEEKS, Math.ceil(totalDays / WEEK_LEN))
    ...
    // Two schema mismatches fixed here:
    //  - study_plan_goals.title is NOT NULL with no default.
    //  - priority has CHECK (priority IN ('low','medium','high')), but the
    //    agent's prompt never constrains what it returns for that field. A
    //    single multi-row insert is atomic, so one out-of-range value would
    //    silently fail ALL goals at once. Normalised rather than trusted.
    ...
    for (let day = 0; day < totalDays; day++) {
      // Tiling: day 8 reuses schedule[0], day 9 reuses schedule[1], etc. —
      // the same weekly rhythm repeated for spaced review, not the same
      // single day repeated four times.
      const item = isWeeklyTemplate ? schedule[day % schedule.length] : schedule[day]
      if (!item) continue

      const due = new Date(startDate)
      due.setDate(due.getDate() + day)
      const subtopics = Array.isArray(item?.subtopics) ? item.subtopics.join(", ") : ""
      const weekNum = Math.floor(day / WEEK_LEN) + 1

      goals.push({
        plan_id: createdPlan.id,
        title: item?.topic || `Day ${day + 1}`,
        description: subtopics || null,
        due_date: due.toISOString().slice(0, 10),
        // estimated_hours is INTEGER, so it must be whole. Rounding to 1dp
        // sent "1.5" for a 90-minute day, which Postgres rejects. Minimum 1
        // so a short session isn't stored as 0 hours.
        estimated_hours: item?.timeMinutes ? Math.max(1, Math.round(Number(item.timeMinutes) / 60)) : null,
        priority: normalisePriority(item?.priority),
        is_review: false,
        // carried through only for the tasks-mirroring step below, stripped
        // before the actual DB insert (study_plan_goals has no week_number
        // column — it's derived from due_date, not stored).
        __week: weekNum,
      })
    }

    // One review/exam checkpoint at the end of each completed week — this is
    // what the weekly-review feedback loop (see the review-complete route)
    // reads to know a week is "done" and worth analysing.
    for (let w = 1; w <= weekCount; w++) {
      const weekEndDayIndex = Math.min(w * WEEK_LEN, totalDays) - 1
      if (weekEndDayIndex < 0) continue
      const due = new Date(startDate)
      due.setDate(due.getDate() + weekEndDayIndex)
      goals.push({
        plan_id: createdPlan.id,
        title: `Week ${w} Review & Practice Exam`,
        description:
          "Look back on this week's topics, sit a short practice set on what you covered, and mark this complete to get feedback on what to focus on next week.",
        due_date: due.toISOString().slice(0, 10),
        estimated_hours: 1,
        priority: "high",
        is_review: true,
    ...
    // plan_goal_id can't be set until the goal row exists, so goals are
    // inserted first, then mirrored into tasks as a second pass using the
    // ids Postgres just generated.
    const goalRows = goals.map(({ __week, ...g }) => g)
    const { data: insertedGoals, error: goalsError } = await supabase
      .from("study_plan_goals")
      .insert(goalRows)
      .select("id, title, description, due_date, estimated_hours, priority, is_review")
    ...
    // Mirror into `tasks` — this is the actual fix for "exported plans don't
    // show on the calendar". The Planner page's Calendar and Task List read
    // ONLY from `tasks`, never from study_plan_goals directly, so without
    // this step an export is invisible outside the "My Study Plans" tab.
    const taskRows = insertedGoals.map((g) => ({
      user_id: user.id,
      title: g.title,
      description: g.description,
      due_date: g.due_date, // DATE → TIMESTAMPTZ: Postgres reads this as that date's midnight UTC.
      priority: g.priority,
      subject,
      estimated_hours: g.estimated_hours,
      is_review: g.is_review,
      plan_goal_id: g.id,
      plan_id: createdPlan.id,
      completed: false,
    }))
    ...
    if (tasksError) {
      console.error("[study-agent/export] task mirror failed:", tasksError.message)
      // Not fatal: the plan and its goals both saved successfully and are
      // fully usable from "My Study Plans" — only the calendar mirror
      // failed, so say so precisely rather than reporting total failure.
      return NextResponse.json({
        ok: true,
        target,
        planId: createdPlan.id,
        goalCount: insertedGoals.length,
        weekCount,
