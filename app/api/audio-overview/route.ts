...
import { describeCurriculumForPrompt } from "@/lib/services/syllabus"
...
Target length: approximately ${wordTarget} words. Treat this as a guide, not a hard floor — a complete script that ends properly is far more important than hitting the exact count. Always finish with the concluding summary below; never let the script cut off mid-sentence or mid-thought.
...
${documentText.slice(0, 12000)}`
    ...
    const curriculum = await describeCurriculumForPrompt(user.id).catch(() => null)

    const response = await aiChatCompletion({
      messages: [
        { role: "system", content: `${curriculum ? curriculum + "\n\n" : ""}You are an expert educational content writer who creates clear, engaging audio scripts for students. Return only the spoken script text, no formatting symbols.` },
        { role: "user", content: prompt },
      ],
      temperature: 0.65,
      // Was wordTarget * 2. English runs ~1.3-1.5 tokens per word even
      // before any overshoot, so a 400-word target (short/"Quick") had only
      // ~800 tokens to work with — barely enough for the words alone, with
      // zero margin for the model running long. The script was hitting this
      // ceiling before reaching its own concluding summary and getting cut
      // off mid-sentence, which is exactly what was reported. 3x plus a
      // fixed buffer gives real headroom at every duration.
      max_tokens: wordTarget * 3 + 200,
    }, {
      task: "summarization",
      agent: "study_ai",
      // "summarization" is cacheable by default, which is right for e.g.
      // analyze-syllabus (same input → same output every time). It's WRONG
      // here: the gateway's semantic cache only embeds the first ~2000
      // characters of the prompt, and this prompt puts the document content
      // first — so the Style/Length instructions further down never
      // actually influence the cache lookup. Two requests on the same notes
      // with different style/duration settings were hashing to (near-)
      // identical embeddings and getting back the exact same cached script,
      // regardless of what was actually asked for. Explicitly uncacheable.
      cacheable: false,
    })
