...
import { describeCurriculumForPrompt, buildSyllabusContentFromCurriculum } from "@/lib/services/syllabus"
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Two valid sources: pasted/uploaded content, or the learner's syllabus —
    // same pattern as Practice Exams, Mock Exam, Essay and Study Agent.
    let content = rawContent
    if (fromSyllabus && (!content || !content.trim()) && subject) {
      content = await buildSyllabusContentFromCurriculum(syllabus, subject, topicFocus).catch(() => null)
      if (!content) {
        return NextResponse.json({ error: "Could not find curriculum for that subject" }, { status: 400 })
      }
    }
    ...
    // Scope cards to the learner's syllabus and subjects — same prompt for
    // both the vision and text paths below.
    const curriculum = await describeCurriculumForPrompt(authedUser.id).catch(() => null)
    const scopedSystemPrompt = `${curriculum ? curriculum + "\n\n" : ""}${buildSystemPrompt(mode, cardTypes)}`
    ...
    const response = await aiChatCompletion(
      ...
      },
      {
        ...
        task: "flashcards",
        agent: "study_ai",
        // "flashcards" is in CACHEABLE_TASKS with a 7-DAY TTL. The semantic
        // cache only embeds the first ~2000 characters of the prompt, and
        // with this prompt that window is dominated by shared boilerplate
        // ("Generate exactly N flashcards... Mix these card types...") —
        // the actual subject-specific content sits further down. Two
        // requests for genuinely different subjects were landing close
        // enough in that embedding to collide, so every syllabus-based
        // generation kept returning whichever subject's deck got cached
        // first (Agricultural Science, in this case) regardless of what was
        // actually selected. Same bug already found and fixed in
        // app/api/audio-overview/route.ts.
        cacheable: false,
      },
    )
