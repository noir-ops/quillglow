...
{"root":{"id":"root","label":"Topic","description":"Overview","children":[{"id":"b1","label":"Branch","description":"Info","children":[{"id":"l1","label":"Leaf","description":"Detail"}]}]}}`
    ...
    // Scope generation to the learner's syllabus and subjects so topic
    // names, depth and terminology match their curriculum.
    ...
    const response = await aiChatCompletion({
      messages: [
        { 
          role: "system", 
          content: `${curriculum ? curriculum + "\n\n" : ""}You are a JSON generator. Return ONLY valid JSON, no markdown code blocks, no explanations.` 
        },
      ...
      task: "mind_map",
      agent: "study_ai",
      // Same semantic-cache collision bug already found and fixed in
      // audio-overview and generate-flashcards: "mind_map" is a cacheable
      // task, and this prompt's distinguishing topic/subject content sits
      // past the portion the cache actually embeds. Left as-is, two
      // different topics could collide and this would silently generate the
      // same mind map for both.
      cacheable: false,
    })
