...
const GRAMMAR_SYSTEM = `You are an expert proofreader and grammar checker for student writing. Find every grammar, spelling, punctuation and word-choice error in the text. Return ONLY a valid JSON object — no markdown, no explanation, no preamble.

Return this exact JSON shape:
{
  "corrected": "<the full text with every error fixed, otherwise unchanged>",
  "issues": [
    {"type": "<Grammar|Spelling|Punctuation|Word Choice|Clarity>", "original": "<the exact problem snippet>", "fix": "<the corrected snippet>", "explanation": "<one short sentence>"}
  ],
  "issue_count": <integer — total issues found>,
  "clarity_notes": ["<short phrase>", "<short phrase>"]
}
Do not rewrite the student's voice, style or arguments — fix errors only. If a sentence is already correct, leave it untouched.`

const PARAPHRASE_SYSTEM = `You are an expert academic paraphraser. Rewrite the given text so it expresses the exact same meaning in substantially different wording and sentence structure — enough to pass a plagiarism/similarity check — while keeping every fact, argument and conclusion intact. Return ONLY a valid JSON object — no markdown, no explanation, no preamble.

Modes:
- standard: natural rewrite, same reading level
- fluent: smoother, more polished prose
- academic: formal register, precise vocabulary
- simple: shorter sentences, plainer vocabulary — for a younger reading level

Return this exact JSON shape:
{
  "paraphrased": "<the full rewritten text>",
  "similarity_estimate": <integer 0-100 — estimated % of phrasing still matching the original>,
  "changes_made": ["<short phrase>", "<short phrase>"]
}
Never drop or alter facts, numbers, names or the argument's meaning — only the wording and structure change.`
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Detection is analysis-only and stays free. Humanize, grammar and
    // paraphrase all produce a full rewrite — same AI cost class as
    // humanize — so they share the same free-tier gate. Each mode tracks
    // its own 3/month allowance independently (one doesn't eat into
    // another's quota).
    ...
    if (!isGenius && GENERATIVE_MODES.includes(mode)) {
      ...
      if ((count ?? 0) >= FREE_MONTHLY_LIMIT) {
        return NextResponse.json({
          ...
          message: `You've used all ${FREE_MONTHLY_LIMIT} WriteReal ${mode}s this month. Upgrade to Genius for unlimited access.`,
        }, { status: 429 })
      }
    }
    ...
    if (mode === "grammar") {
      ...
      const raw = await callAI(GRAMMAR_SYSTEM, userPrompt, 0.3)

      let result: any
      try {
        result = parseJSON(raw)
      } catch {
        return NextResponse.json({ error: "Failed to parse grammar check result" }, { status: 500 })
      }
      ...
      await supabase.from("writing_improver_logs").insert({
        ...
        mode: "grammar",
        original_text: text.slice(0, 15000),
        improved_text: result?.corrected?.slice(0, 15000) ?? null,
        tone: null,
        explanation: result,
      })
    ...
    if (mode === "paraphrase") {
      const style = (options.style as string) || "standard"
      const userPrompt = `Paraphrase mode: ${style}\n\nText to paraphrase:\n${text.slice(0, 6000)}`
      const raw = await callAI(PARAPHRASE_SYSTEM, userPrompt, 0.7)

      let result: any
      try {
        result = parseJSON(raw)
      } catch {
        return NextResponse.json({ error: "Failed to parse paraphrase result" }, { status: 500 })
      }
      ...
      await supabase.from("writing_improver_logs").insert({
        ...
        mode: "paraphrase",
        original_text: text.slice(0, 15000),
        improved_text: result?.paraphrased?.slice(0, 15000) ?? null,
        tone: style,
        explanation: result,
      })
