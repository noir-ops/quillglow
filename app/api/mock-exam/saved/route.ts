...
/**
 * Delete a saved mock exam attempt.
 *
 * Scoped to the signed-in user via the user_id filter, so an id from another
 * account can't be deleted even if guessed.
 */
export async function DELETE(req: Request) {
  ...
  const { error } = await supabase.from("mock_exam_attempts").delete().eq("id", id).eq("user_id", user.id)

  if (error) {
    console.error("[mock-exam/saved] delete failed:", error.message)
