...
Return ONLY JSON in exactly this shape:
{
  "questions": [
    {
      "question": "A leaf is adapted for photosynthesis. Which feature helps gases diffuse rapidly into the leaf?",
      "options": [
        "A thin, flat blade giving a short diffusion path",
        "A thick waxy cuticle on the upper surface",
        "Densely packed palisade cells",
        "A deep layer of xylem in the midrib"
      ],
      "type": "single",
      "correctAnswer": "A",
      "explanation": "A thin blade shortens the diffusion distance, so gases reach the mesophyll faster.",
      "difficulty": "easy"
    }
  ]
}
...
- Each entry MUST be the full answer text a student reads and chooses between.
- Do NOT return letters ("A", "B", "C", "D") as the option values, and do NOT
  prefix the text with a letter or number — the interface adds the A/B/C/D
  labels itself from position.
- Provide exactly 4 options unless the question type requires otherwise.
- "correctAnswer" is the LETTER of the correct option by position, where the
  first option is "A", the second "B", and so on.
`
    ...
    const { data: attempt, error: attemptError } = await supabase
      ...
      .insert({
        ...
        difficulty,
        time_limit_minutes: timeLimit,
        // Null for document-generated papers; set when built from curriculum.
        subject: fromSyllabus ? subject : null,
        syllabus: fromSyllabus ? (syllabus ?? null) : null,
    ...
    if (attemptError) {
      ...
      return NextResponse.json(
        { error: "Failed to create attempt", detail: attemptError.message },
        { status: 500 },
      )
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      gradedAnswers,
      // Was omitted, so the results screen read undefined and rendered
      // "NaN:NaN" for Time Taken.
