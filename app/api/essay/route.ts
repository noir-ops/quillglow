...
export async function POST(req: NextRequest) {
  try {
    ...
    // same pattern as Practice Exams and Mock Exam. "subject" (a real
    // learning_concepts subject, e.g. "Biology") is what the curriculum is
    // pulled for; "topic" stays the essay's own theme within that subject
    // (e.g. "Photosynthesis") and is never used for the curriculum lookup —
    // it's usually too granular to match a subject name.
    let documentText = rawDocumentText
    if (fromSyllabus && (!documentText || !documentText.trim()) && subject) {
      documentText = await buildSyllabusContentFromCurriculum(syllabus, subject, topic).catch(() => null)
      if (!documentText) {
