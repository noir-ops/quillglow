...
export function EssayWriter({ onBack }: EssayWriterProps) {
  ...
  // Source of the study material. "syllabus" builds from the learner's
  ...
  const handleGenerateQuestions = async () => {
    ...
    if (!extractedText && !readyViaSyllabus) {
      toast.error(source === "syllabus" ? "Please choose a subject" : "Please upload a document")
      return
    }
    ...
    try {
      ...
      const response = await fetch("/api/essay", {
        ...
        body: JSON.stringify({
          ...
          documentText: source === "syllabus" ? "" : extractedText,
          fromSyllabus: source === "syllabus",
          subject: source === "syllabus" ? subject : undefined,
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                {source === "syllabus"
                  ? "Questions are generated from your syllabus's indexed curriculum."
                  : "Upload a PDF document to generate essay questions from."}
              </p>
            </CardHeader>
              ...
              {source === "syllabus" && mySubjects.length > 0 ? (
                <div>
                  ...
                  <select
                    ...
                    className="mt-1.5 w-full h-9 px-3 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                  >
                    {mySubjects.map((m) => (
            ...
            <CardContent className="space-y-4">
              ...
              <Button
                ...
                disabled={
                  loading ||
                  !topic ||
                  (source === "syllabus" && mySubjects.length > 0 ? !subject : !extractedText)
                }
