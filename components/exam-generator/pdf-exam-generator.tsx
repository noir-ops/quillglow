...
export function PDFExamGenerator() {
  ...
  // Source of the questions. "syllabus" builds straight from the learner's
  ...
  // Which option the learner picked for each MCQ, keyed by question index.
  // Nothing here means nothing revealed — this is what makes the answer
  // actually withheld until a choice is made, instead of being shown the
  ...
  // The learner's subjects, from their selected syllabi. Only shown as a
  // generation source when there's actually something to generate from.
  useEffect(() => {
  ...
  const handleGenerate = async (isRegenerate = false) => {
    const readyViaSyllabus = source === "syllabus" && mySubjects.length > 0 && !!subject
    if (!extractedText.trim() && !readyViaSyllabus) {
      toast.error(source === "syllabus" ? "Please choose a subject" : "Please upload a PDF first")
      return
    }
    ...
    try {
      ...
      const response = await fetch("/api/pdf/exam-questions", {
        ...
        body: JSON.stringify({
          // In syllabus mode there's nothing uploaded — the route builds the
          // content from the subject's indexed curriculum instead.
          text: source === "syllabus" ? "" : extractedText,
          ...
          subject,
          pdfFileName: source === "syllabus" ? null : pdfFileName,
          settings,
          generationId,
          seed,
  ...
  // Reset on every new result (fresh generation or opening a different saved
  // exam) — otherwise a previous exam's answers would carry over and a
  // question could show as already-answered before it's ever been seen.
  useEffect(() => {
    setMcSelections({})
  }, [result])

  // Record the practice exam's score once EVERY MCQ has been answered, so it
  // counts toward the combined Mock Exam Score. Previously the grading only
  // ever lived on screen. Guarded per result so revisiting or re-rendering
  // the same exam never records it twice.
  const recordedResultRef = useRef<ExamResult | null>(null)
  // Which exam the current answers were actually given on. When a new exam
  // loads, the answer-reset effect and this one run in the same render, so
  // without this the previous exam's full answer set could be scored
  // against the new exam for one render.
  const answersForResultRef = useRef<ExamResult | null>(null)
  useEffect(() => {
    const mcqs = result?.multipleChoice ?? []
    if (!result || mcqs.length === 0) return
    if (answersForResultRef.current !== result) return
    if (recordedResultRef.current === result) return
    if (Object.keys(mcSelections).length < mcqs.length) return

    recordedResultRef.current = result
    const correct = mcqs.filter((q, i) => mcSelections[i] === q.correctAnswer).length
    fetch("/api/practice-exam/score", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ correct, total: mcqs.length, subject: subject || null, examId: result.examId || null }),
    })
      .then((r) => {
        if (r.ok) toast.success(`Scored ${correct}/${mcqs.length} — added to your Mock Exam Score`)
      })
      .catch(() => {})
  }, [mcSelections, result, subject])

  const selectMCAnswer = (questionIndex: number, letter: string) => {
    // Locked once answered — re-clicking to see every option turn green
    // would defeat the point of grading it at all.
    answersForResultRef.current = result
    setMcSelections((prev) => (prev[questionIndex] ? prev : { ...prev, [questionIndex]: letter }))
  }
  ...
  return (
    <div className="space-y-6">
      {/* Genius Plan / usage — moved to the very top of the page. */}
      {usage && (
        <Card
          className={cn(
            "border-2",
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {/* Source selector — only shown when the learner has subjects set,
              otherwise document upload is the only real option. */}
          {mySubjects.length > 0 && (
            <div className="grid grid-cols-2 gap-2">
              <Button
                type="button"
          ...
          {/* Settings + generate. Previously gated on pdfFileName alone,
              which meant the syllabus path had no visible settings and no
              Generate button — there's no upload in that flow. */}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={
                    loading ||
                    (source === "syllabus" && mySubjects.length > 0 ? !subject : !extractedText.trim()) ||
                    totalQuestions === 0 ||
            ...
            {result.multipleChoice && result.multipleChoice.length > 0 && (
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Pick an answer to see if you got it right — nothing is revealed until you do.
                  </p>
                </CardHeader>
                <CardContent className="space-y-3">
                  {result.multipleChoice.map((q, index) => {
                    // Letter derived from POSITION, not the option text — the
                    // options are prefixed ("A) ...") by the prompt, but
                    // deriving from index is the same defensive pattern used
                    // for Mock Exam and can't be thrown off by a prefix the
                    // model gets wrong or omits.
                    const selected = mcSelections[index]
                            ...
                            <div className="px-4 pb-4 pt-0 space-y-3">
                              <div className="space-y-2 ml-9">
                                {q.options.map((option, optIndex) => {
                                  ...
                                  const isThisCorrect = optionLetter === q.correctAnswer

                                  // Before an answer is picked: every option
                                  // looks identical — no color, no hint of
                                  // which one is right. That's the entire
                                  // fix; the old version highlighted the
                                  // correct option unconditionally.
                                  let style = "bg-muted/50 hover:bg-muted cursor-pointer"
                                  if (answered) {
                                    if (isThisCorrect) {
                                      style = "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 font-medium"
                                    } else if (isThisSelected) {
                                      style = "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 font-medium"
                                    } else {
                                      style = "bg-muted/30 opacity-60"
                                    }
                                  }
                                  ...
                                  return (
                                    <button
                                      key={optIndex}
                                      type="button"
                                      onClick={() => selectMCAnswer(index, optionLetter)}
                                      disabled={answered}
                                      className={cn(
                                        "w-full text-left p-2 rounded text-sm transition-colors flex items-center justify-between gap-2",
                                        style,
                                        answered && "cursor-default",
                                      )}
                                    >
                                      <span>{option}</span>
                                      {answered && isThisCorrect && <CheckCircle className="h-4 w-4 flex-shrink-0" />}
                              ...
                              {/* Explanation only appears once answered —
                                  showing it earlier would give the answer
                                  away just as much as highlighting the
                                  option did. */}
                              {answered ? (
                                ...
                                <p className="ml-9 text-xs text-muted-foreground italic">
                                  Select an answer above to see if you're right.
                                </p>
                              )}
                            </div>
                          </CollapsibleContent>
