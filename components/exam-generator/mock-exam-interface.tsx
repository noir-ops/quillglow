...
export function MockExamInterface({ attemptId, questions, totalQuestions, timeLimit, onExit, originalQuestions }: MockExamInterfaceProps) {
  ...
  const formatTime = (seconds: number | null) => {
    ...
    // Guard against undefined/NaN reaching this — it previously rendered
    // literally as "NaN:NaN" on the results screen.
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {questions[currentQuestion].options.map((option, idx) => {
                // Derive the letter from POSITION, not from the option text.
                // The old `option.charAt(0)` assumed every option was prefixed
                // ("A. 16 g mol-1"), but generated options are often bare
                // ("16 g mol-1", "17 g mol-1"), so three options all yielded
                // "1" — they shared an identity and selecting one highlighted
                // all of them.
                ...
                return (
                  ...
                  >
                    {/* Label comes from position, so it stays correct even if
                        the model ever prefixes the text itself. */}
                    <span
                      className={`shrink-0 h-6 w-6 rounded-full grid place-items-center text-xs font-bold ${
