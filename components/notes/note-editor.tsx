...
import { NoteContentView, parseStructuredNote } from "@/components/notes/note-content-view"
...
export function NoteEditor({ note: initialNote }: NoteEditorProps) {
  ...
  // Notes generated by Study Agent open in a rendered Preview by default —
  // that's the whole fix, since the raw textarea showed literal "##" and
  // "**" markdown syntax instead of the same styled sections Study Agent's
  // own Notes tab uses. Anyone can still switch to Edit. A note that
  // doesn't match that structure just opens straight into Edit, unchanged.
  const [mode, setMode] = useState<"edit" | "preview">(
    parseStructuredNote(initialNote.content || "") ? "preview" : "edit",
  )
  ...
  useEffect(() => {
    ...
    setMode(parseStructuredNote(initialNote.content || "") ? "preview" : "edit")
    fetchAllNotes()
  }, [initialNote])
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {mode === "preview" ? (
          parseStructuredNote(note.content || "") ? (
            // Structured view has its own expand/collapse buttons per
            // section — no click-to-edit wrapper here, or every attempt to
            // open a section would immediately bounce into Edit mode.
            <NoteContentView content={note.content || ""} />
          ) : (
            <div onClick={() => setMode("edit")} className="cursor-text min-h-[500px]">
              {note.content ? (
                <NoteContentView content={note.content} />
              ) : (
                <p className="text-muted-foreground">Start typing your notes...</p>
              )}
            </div>
          )
        ) : (
