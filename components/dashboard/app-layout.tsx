...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { href: "/study-agent", icon: Zap, label: "StudyPilot", isNew: true },
    ...
    },
    {
      ...
      items: [
        ...
        { href: "/audio-overview", icon: Headphones, label: "LearnCast", isNew: true },
  ...
  return (
    <div id="app-theme-root" className="min-h-screen bg-background">
      ...
      {/*
        min-w-0 is the actual fix. A flex-1 item defaults to min-width:auto,
        so it refuses to shrink below its widest unbreakable child (a long
        deck title, a row of icon buttons, a table). On desktop there's room
        and nobody notices; on a tablet, with 256px already taken by the
        fixed sidebar, one wide element anywhere on a page pushed the whole
        content column past the right edge. The page then scrolled sideways
        and the fixed sidebar slid over the content's left edge — which is
        why the same break appeared on many unrelated pages at once.

        overflow-x-clip is a guardrail on top: if any individual component
        still overflows, it's clipped at the content column instead of
        making the entire page scroll horizontally. `clip` rather than
        `hidden` so it doesn't create a scroll container, which would break
        position:sticky headers inside pages.
      */}
      <div
      ...
      >
        <main className="flex-1 min-w-0 overflow-x-clip pt-14 md:pt-0 pb-28 md:pb-8">{children}</main>
      </div>
    </div>
  )
}
