...
export function AIFlashcardGenerator({ deckId, subject: deckSubject, onGenerated }: AIFlashcardGeneratorProps) {
  const [inputMode, setInputMode] = useState<"syllabus" | "text" | "pdf" | "image" | "saved">("text")
  ...
  // The learner's subjects, from their selected syllabi.
  useEffect(() => {
    fetch("/api/syllabus/subjects")
      ...
      .then((d) => {
        ...
        if (list.length > 0) {
          ...
          setSyllabusSubject((prev) => prev || list[0].subject)
        }
      })
      .catch(() => {})
  }, [])
  ...
  const handleGenerate = async () => {
    ...
    if (inputMode === "syllabus") {
      if (!syllabusSubject) { toast.error("Please choose a subject"); return }
      // No content to prepare client-side — the route builds it from the
      // subject's indexed curriculum. finalContent stays empty on purpose.
      finalContent = ""
    ...
    setLoading(true)
    try {
      const owningSyllabus = mySubjects.find((s) => s.subject === syllabusSubject)?.syllabus
      const response = await fetch("/api/ai/generate-flashcards", {
        ...
        body: JSON.stringify({
          ...
          deckId,
          // In syllabus mode the picked curriculum subject IS the subject —
          // otherwise fall back to the deck's own subject, as before.
          subject: inputMode === "syllabus" ? syllabusSubject : deckSubject,
          fromSyllabus: inputMode === "syllabus",
          syllabus: inputMode === "syllabus" ? owningSyllabus : undefined,
          topicFocus: inputMode === "syllabus" ? topicFocus.trim() || undefined : undefined,
          cardCount,
          mode,
          cardTypes,
  ...
  const canGenerate = () => {
    ...
    if (inputMode === "syllabus") return !!syllabusSubject
        ...
        <CardContent className="space-y-5">
          ...
          <Tabs value={inputMode} onValueChange={(v) => setInputMode(v as typeof inputMode)}>
            <TabsList className="grid w-full grid-cols-5">
              <TabsTrigger value="syllabus" className="flex items-center gap-1.5 text-xs">
            ...
            <TabsContent value="syllabus" className="mt-3 space-y-3">
              {mySubjects.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No subjects selected yet — pick a syllabus and subjects on your dashboard first.
                </p>
              ) : (
                <>
                  <div className="space-y-1.5">
                    ...
                    >
                      {mySubjects.map((m) => (
                        <option key={`${m.syllabus}::${m.subject}`} value={m.subject}>
                          {m.subject} ({m.syllabus})
                        </option>
                      ))}
                    </select>
                    <p className="text-xs text-muted-foreground">
                      Cards follow this syllabus's terminology and style — nothing to paste.
                    </p>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      Topic focus <span className="font-normal text-muted-foreground">(optional)</span>
                    </Label>
                    <Input
                      value={topicFocus}
