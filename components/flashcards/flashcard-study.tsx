...
export function FlashcardStudy({ deck, flashcards: initialFlashcards, allTags: initialTags = [], refreshRef }: FlashcardStudyProps) {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!confirm(`Delete ${selectedCards.size} card${selectedCards.size !== 1 ? "s" : ""}? This can't be undone.`)) {
      return
    }
    ...
    try {
      ...
      // One request per card reuses the existing single-delete endpoint
      // rather than adding a second bulk-delete route for what the Move
      // dialog already treats as a small, in-memory selection (max one
      // deck's worth of cards).
      const results = await Promise.allSettled(
        ids.map((id) => supabase.from("flashcards").delete().eq("id", id)),
      )
      ...
      if (succeeded > 0) {
        ...
        toast.success(`Deleted ${succeeded} card${succeeded !== 1 ? "s" : ""}`)
      }
      if (failed > 0) {
        toast.error(`${failed} card${failed !== 1 ? "s" : ""} could not be deleted`)
      }

      setSelectedCards(new Set())
      if (succeeded === ids.length) setMoveOpen(false)
    } catch (error) {
      console.error("[flashcards] bulk delete failed:", error)
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-3">
            {/* min-w-0 + flex-1 is what makes `truncate` actually work here —
                on a flex child, truncate is silently ignored without it, so a
                long deck name pushed the icon buttons off the right edge on
                tablet. The full name stays available on hover via title. */}
            <div className="flex items-center justify-between gap-2">
              <CardTitle className="text-base sm:text-lg truncate min-w-0 flex-1" title={deck.name}>{deck.name}</CardTitle>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Select cards to move or delete</p>
                </div>
                <div className="flex items-center gap-2">
