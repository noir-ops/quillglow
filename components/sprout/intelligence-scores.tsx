...
interface Score {
  ...
  computed_at: string
  // Set only on Exam Readiness rows — which of the learner's two selected
  // syllabi this card belongs to.
...
const LABELS: Record<string, { name: string; hint: string; inverted?: boolean }> = {
  ...
  mock_exam: { name: "Mock Exam Score", hint: "Average across all exams you've taken" },
        ...
        return (
          <Card key={`${s.score_type}:${s.scope}`}>
            ...
            <CardContent className="space-y-2">
              {s.score_type === "mock_exam" && Number(s.breakdown?.exams_taken ?? 0) === 0 ? (
                // No exams yet — a red 0/100 would read as failing, not "nothing taken".
