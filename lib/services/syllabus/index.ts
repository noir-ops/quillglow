...
/**
 * The topic list (depth 1) for a subject, with how many concepts (depth 2)
 * sit under each. This is what lets the Study Agent generate a study system
 * for a subject the learner has already selected, with nothing pasted —
 * the curriculum itself supplies the content instead of free text.
 */
export async function listTopicsForSubject(syllabus: string, subject: string): Promise<TopicOption[]> {
  const supabase = await createClient()
  ...
  return topics.map((t: any) => ({ name: t.name, conceptCount: counts.get(t.id) ?? 0 }))
}

/**
 * A synthesized "content" block for when the learner has selected a subject
 * but pasted nothing — the topic list stands in for a syllabus paste, so
 * generation has something concrete to work from instead of just a subject
 * name (which is too thin to build a full study system from).
 */
export async function buildSyllabusContentFromCurriculum(
  syllabus: string,
  subject: string,
  topicFocus?: string,
): Promise<string | null> {
  const topics = await listTopicsForSubject(syllabus, subject)
  if (topics.length === 0) return null

  const list = topics.map((t) => `- ${t.name}`).join("\n")
  const focusLine = topicFocus
    ? `\n\nThe learner wants to focus specifically on: ${topicFocus}. Prioritise that within the topics below, but you may draw on the full list for context.`
    : ""

  return `${syllabus} ${subject} — full topic list from the indexed curriculum:\n${list}${focusLine}`
}
