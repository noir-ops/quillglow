...
export interface AICallOptions {
  ...
  userId?: string | null
  /**
   * Explicitly opt a call OUT of gateway caching even though its task is in
   * CACHEABLE_TASKS. Needed when the task is cacheable in general but THIS
   * call's output depends on something the cache can't see — e.g. a style
   * or length parameter that sits outside the ~2000-char window the
   * semantic cache embeds, so two genuinely different requests (same source
   * text, different settings) would otherwise collide and return an
   * identical cached response. Default (omitted) leaves the task's normal
   * cacheability untouched.
   */
  cacheable?: boolean
}
